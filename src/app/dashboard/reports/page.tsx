  ...
  const handleExportPDF = () => {
    ...
    if (allCoFounderStats) {
      allCoFounderStats.forEach(f => {
        data.push([`Equity: ${f.name}`, formatCurrency(f.totalInvestedBDT, "BDT"), "Total Share"]);
      });
    }

    exportToPDF("Executive Financial Summary", headers, data, `QuoteX_Report_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">High-level financial summaries and performance trends.</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={handleExportCSV}
            className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-gray-400 hover:text-white hover:border-white/20 transition-all uppercase tracking-widest"
          >
            <Download className="w-3.5 h-3.5" /> CSV
          </button>
          <button 
            onClick={handleExportPDF}
            className="flex items-center gap-2 px-4 py-2 bg-blue-600/10 border border-blue-600/20 rounded-xl text-[10px] font-black text-blue-500 hover:bg-blue-600 hover:text-white transition-all uppercase tracking-widest shadow-lg shadow-blue-500/5"
          >
            <Download className="w-3.5 h-3.5" /> Export PDF
          </button>
          <div className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2">
            <ShieldCheck className="w-4 h-4 text-emerald-500" />
            <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest">Real-Time Data Audited</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-[#0f0f0f] border border-white/5 rounded-3xl p-8 space-y-8 flex flex-col justify-between">
                ...
                <div key={founder.userId} className="space-y-2">
                  <div className="flex items-center justify-between text-[9px] font-black uppercase tracking-widest text-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-400">{founder.name.split(" ")[0]} Capital</span>
                    </div>
                    <span className="text-white font-mono flex items-center gap-1.5">
                      {formatCurrency(founder.totalInvestedBDT, "BDT")}
                      <span className="text-[7px] text-gray-500 font-bold opacity-60">
          ...
          <div className="pt-6 border-t border-white/5">
            <Link 
              ...
              className="bg-white/5 rounded-2xl p-4 flex items-center justify-between group cursor-pointer hover:bg-white/10 transition-all w-full"
            >
               <div className="flex items-center gap-3">
                 <div className="w-8 h-8 rounded-lg bg-indigo-500/20 flex items-center justify-center text-indigo-500">
                   <Layers className="w-4 h-4" />
                 </div>
                 <span className="text-sm font-semibold text-gray-300">Detailed Audit</span>
               </div>
      ...
      {(data.allCoFounderStats || data.coFounderStats) && (
        <div className="space-y-6">
          <div className="flex items-center justify-between px-2">
            ...
            <Link href="/dashboard/investments" className="text-[10px] font-black text-gray-500 hover:text-primary transition-colors uppercase tracking-widest flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {(data.allCoFounderStats || (data.coFounderStats ? [data.coFounderStats] : [])).map((founder: any) => (
              <div key={founder.userId} className="p-6 bg-[#050B18] border border-white/5 rounded-3xl relative overflow-hidden group hover:border-primary/20 transition-all shadow-xl shadow-blue-900/5">
                <div className="absolute top-0 right-0 w-24 h-24 bg-primary/5 rounded-full blur-2xl -mr-12 -mt-12 group-hover:bg-primary/10 transition-colors" />
                
                <div className="flex items-center gap-4 mb-6 relative">
                  <div className="w-14 h-14 rounded-2xl bg-primary/10 border border-white/10 flex items-center justify-center text-primary font-black text-xl overflow-hidden group-hover:scale-105 transition-transform duration-500">
                    {founder.image ? (
                      <Image 
                        src={founder.image} 
                        alt={founder.name} 
                  ...
                  <div>
                    <h4 className="text-lg font-black text-white group-hover:text-primary transition-colors tracking-tight">{founder.name}</h4>
                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">Stakeholder</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4 relative">
                  <div className="p-3 bg-black/40 border border-white/5 rounded-2xl">
                    <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-1">Total Share</p>
                    <p className="text-sm font-black text-emerald-400">{formatCurrency(founder.totalInvestedBDT, "BDT")}</p>
                  </div>
                  <div className="p-3 bg-black/40 border border-white/5 rounded-2xl">
                    <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-1">Commitment</p>
                    <p className="text-sm font-black text-rose-400">{formatCurrency(founder.totalPendingDuesBDT, "BDT")}</p>
                  </div>
                </div>
              </div>
            ))}
