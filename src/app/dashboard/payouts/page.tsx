    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">Member Payouts</h1>
            <p className="text-gray-500 text-sm">Disburse salaries, bonuses, and project revenue shares.</p>
          </div>
        </div>
      ...
      <div className="bg-[#050B18] border border-white/5 rounded-3xl overflow-hidden shadow-xl shadow-blue-900/5">
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-3 bg-black/40 border border-white/5 rounded-xl px-4 py-2 w-full max-w-sm">
            ...
            <input type="text" placeholder="Search by member or project..." className="bg-transparent text-sm text-white placeholder:text-gray-700 outline-none w-full font-medium" />
          </div>
        </div>
          ...
          <table className="w-full text-center border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                ...
                <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Recipient</th>
                <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Category</th>
                <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Disbursement</th>
                <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Operational Link</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-500">No payout records found.</td></tr>
              ) : (
                payouts.map((p) => (
                  <tr key={p._id} className="hover:bg-white/[0.02] transition-colors group">
                    <td className="px-6 py-4 text-sm text-gray-400 whitespace-nowrap text-center font-medium">{formatDate(p.date)}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3 justify-center">
                        {p.recipient?.image ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary/40 to-blue-600/40 flex items-center justify-center text-[10px] font-bold text-white border border-white/10 shrink-0">
                        ...
                        <div className="flex flex-col items-start translate-x-[-2px]">
                          ...
                          <div className="text-[9px] text-gray-500 font-extrabold uppercase tracking-tighter">{p.recipient?.role?.replace("_", " ")}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <span className="px-2.5 py-1.5 rounded-lg bg-blue-500/5 text-blue-400 text-[10px] font-black tracking-widest uppercase border border-blue-500/10">
                        {p.type}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-center">
                      <div className="flex flex-col items-center">
                        <span className="text-sm font-black text-white tracking-widest group-hover:text-blue-400 transition-colors">{formatCurrency(p.amountBDT, "BDT")}</span>
                        <span className="text-[10px] text-gray-600 font-bold">~ {formatCurrency(p.amountUSD, "USD")}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-center">
