...
export default function DashboardPage() {
  ...
  const projectStats = [
    { label: "Active", count: summary?.projectStats?.ACTIVE || 0, color: "text-cyan-400", bg: "bg-cyan-400/10" },
    { label: "Completed", count: summary?.projectStats?.COMPLETED || 0, color: "text-emerald-400", bg: "bg-emerald-400/10" },
    { label: "Canceled", count: summary?.projectStats?.CANCELLED || 0, color: "text-rose-400", bg: "bg-rose-400/10" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Here's a live audit of QuoteXStudio's operations.</p>
        </div>
        <div className="flex items-center gap-3">
          <Link 
            href="/dashboard/ledger"
            className="flex items-center gap-2 px-4 py-2 bg-[#141414] hover:bg-[#1c1c1c] border border-white/5 rounded-xl text-sm font-medium text-gray-300 transition-all font-semibold"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {projectStats.map((stat, i) => (
          <div key={i} className="p-4 bg-[#050B18] border border-white/5 rounded-2xl flex items-center justify-between shadow-md">
            <div className="flex items-center gap-3">
              <div className={cn("w-2 h-2 rounded-full", stat.color.replace("text", "bg"))} />
              <span className="text-xs font-black text-gray-500 uppercase tracking-widest">{stat.label} Projects</span>
            </div>
      ...
      {["SUPER_ADMIN", "CO_FOUNDER"].includes(session?.user?.role as string) && allCoFounderStats && (
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-white px-2">Co-Founder Matrix</h2>
          <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar">
            {allCoFounderStats.map((founder: any) => (
              <div key={founder.userId} className="min-w-[320px] p-6 bg-[#0f0f0f] border border-white/5 rounded-3xl relative overflow-hidden group">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold text-lg overflow-hidden border border-white/10 shrink-0">
                    {founder.image ? (
                      <Image 
                        src={founder.image} 
                        alt={founder.name} 
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-white group-hover:text-primary transition-colors">{founder.name}</h4>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-extrabold group-hover:text-gray-400 transition-colors">Co-Founder</p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-end border-b border-white/5 pb-3">
                    <div>
                      ...
                      <p className="text-lg font-black text-emerald-400">{formatCurrency(founder.totalInvestedBDT, "BDT")}</p>
                    </div>
                    <p className="text-[10px] text-gray-600 font-mono mb-1">({formatCurrency(founder.totalInvestedBDT / (exchangeRate || 120), "USD")})</p>
                  </div>
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">Pending Dues</p>
                      <p className="text-lg font-black text-rose-400">{formatCurrency(founder.totalPendingDuesBDT, "BDT")}</p>
                    </div>
                    <p className="text-[10px] text-gray-600 font-mono mb-1">({formatCurrency(founder.totalPendingDuesBDT / (exchangeRate || 120), "USD")})</p>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-[#050B18] border border-white/5 rounded-2xl overflow-hidden shadow-xl shadow-blue-900/5">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.02]">
                  <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Project Identity</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Stakeholder</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Asset Value</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-500 uppercase tracking-widest text-center">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {activeProjects?.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-8 text-center text-gray-600 text-[10px] font-black uppercase tracking-widest italic">No active projects found.</td></tr>
                ) : (
                  activeProjects?.map((project: any) => (
                    <tr key={project._id} className="hover:bg-white/[0.02] transition-colors group cursor-pointer" onClick={() => router.push(`/dashboard/projects/${project._id}`)}>
                      <td className="px-6 py-4 text-center">
                        <span className="text-sm font-black text-white group-hover:text-blue-400 transition-colors uppercase tracking-tight">{project.title}</span>
                      </td>
                      <td className="px-6 py-4 text-[10px] text-gray-500 font-bold uppercase tracking-widest text-center">{project.clientName || "Direct"}</td>
                      <td className="px-6 py-4 text-center">
                        <div className="text-sm font-black text-white">{formatCurrency(project.totalValueUSD * exchangeRate, "BDT")}</div>
                        <div className="text-[10px] text-gray-600 font-bold group-hover:text-blue-500/50 transition-colors">({formatCurrency(project.totalValueUSD, "USD")})</div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className="px-2 py-1 rounded-md bg-blue-500/5 text-blue-400 text-[9px] font-black uppercase tracking-widest border border-blue-500/10">
                          {project.status}
                        </span>
                      </td>
                    </tr>
        ...
        <div className="space-y-8 flex flex-col">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-[#050B18] border border-white/5 rounded-3xl p-6 relative overflow-hidden shadow-xl shadow-blue-900/5">
                    ...
                    <div key={log._id} className="flex gap-4 items-start relative p-2 rounded-2xl transition-all cursor-pointer" onClick={() => setSelectedLog(log)}>
                      <div className="relative shrink-0">
                        {log.actor?.image ? (
                          ...
                          <div className="w-8 h-8 rounded-full border border-blue-500/20 bg-blue-500/10 text-blue-500 flex items-center justify-center">
