...
export function AuditTraceView({ oldValue, newValue, minimal = false }: AuditTraceViewProps) {
  ...
  return (
    <div className={cn(
      "w-full overflow-hidden rounded-3xl border border-blue-500/20 bg-[#050B18]/60 backdrop-blur-xl transition-all duration-500",
      !minimal && "shadow-[0_0_40px_rgba(59,130,246,0.1)]"
    )}>
      {/* Glow Header */}
      {!minimal && (
        <div className="relative p-6 border-b border-white/5 bg-gradient-to-r from-blue-500/10 via-transparent to-purple-500/10">
      ...
      {/* Panels Container */}
      <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-white/5 h-full">
        {/* Previous State Panel */}
        <div className="flex flex-col">
          <div className="px-6 py-4 bg-white/[0.02] border-b border-white/5">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-rose-500/80">
              Previous State
            </p>
          </div>
          <div className="p-6 space-y-3">
            {diffs.map((diff, idx) => (
              <div key={`old-${idx}`} className="flex flex-col gap-1.5 min-h-[54px] justify-center">
                <span className="text-[9px] font-bold text-gray-600 uppercase tracking-wider">{diff.label}</span>
                <div className="p-2.5 rounded-xl bg-black/40 border border-white/5">
                  <span className={cn(
                    "text-xs font-mono break-all",
                    diff.isChanged ? "text-gray-500 opacity-50 italic" : "text-gray-400"
                  )}>
        ...
        <div className="flex flex-col bg-blue-500/[0.01]">
          <div className="px-6 py-4 bg-white/[0.02] border-b border-white/5">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-emerald-500/80">
              Mutation Result
            </p>
          </div>
          <div className="p-6 space-y-3">
              ...
              <div key={`new-${idx}`} className="flex flex-col gap-1.5 min-h-[54px] justify-center">
                ...
                <div className={cn(
                  ...
                  <span className={cn(
                    "text-xs font-mono break-all",
                    diff.isChanged 
                      ? "text-emerald-500 font-bold drop-shadow-[0_0_8px_rgba(16,185,129,0.3)]" 
                      : "text-gray-400"
                  )}>
